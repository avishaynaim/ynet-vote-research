...
async def main():
    ...
    parser.add_argument("--target", type=int, default=0,
                        help="Stop after this many total discovered (0 = run forever)")
    parser.add_argument("--pause", type=int, default=60, help="Seconds between cycles")
    args = parser.parse_args()

    forever = args.target <= 0
    log(f"Continuous Pipeline — {'running forever' if forever else f'target {args.target}'}")
    ...
    while True:
        ...
        if not forever and len(master_pool) >= args.target:
